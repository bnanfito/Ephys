...
%         trainData = d{p}(strcmp(d{p}.area,area{a}),[3,20,22,26:28]);
        ...
        mdl_ldr{p,a} = fitlme(trainData,'ldr ~ 1+ manipulation + (1+manipulation|animal)','FitMethod','ML');
        [animVals_ldr,animInfo,animStats]=randomEffects(mdl_ldr{p,a});
        mdl_lor{p,a} = fitlme(trainData,'lor ~ 1+ manipulation + (1+manipulation|animal)','FitMethod','ML');
    end
end
...
for p = 1:length(proj)
    ...
    for ar = 1:length(area)
        ...
        for anim = 1:nAnim
            ...
            y2 = d{p}.ldr(idx2);
            scatter(x1,y1,'o','MarkerEdgeColor','none','MarkerFaceColor',animClrs(anim,:),'MarkerFaceAlpha',0.5)
            scatter(x2,y2,'o','MarkerEdgeColor','none','MarkerFaceColor',animClrs(anim,:),'MarkerFaceAlpha',0.5)
            xticks([1:nAnim,nAnim+2:nAnim+1+nAnim])
            xticklabels([animals;animals])
        end
        plot([0.5 nAnim+0.5],repmat(mdl_ldr{p,ar}.Coefficients.Estimate(1),1,2),'k-','LineWidth',2)
...
for p = 1:length(proj)
    ...
    for ar = 1:length(area)
        ...
        plot([0.5 nAnim+0.5],repmat(mdl_lor{p,ar}.Coefficients.Estimate(1),1,2),'k-','LineWidth',2)
