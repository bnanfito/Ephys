...
data(2).age = 'P33-35';
data(2).expList{2,1} =    {'febg7_u000_003';'febg7_u001_004';'febg8_u001_003';'febg8_u002_002';'febg8_u000_000'};
...
figure;
for a = 2:4
    subplot(1,3,a-1,polaraxes);hold on
    clear x y
for c = conds

    x = mean(vertcat(data(a).sums{c}.tuningX{:}));
%     y = mean(vertcat(data(a).sums{c}.tuningY{:}));
    for u = 1:height(data(a).sums{c})

        y(u,:) = data(a).sums{c}.tuningY{u}*data(a).sums{c}.rPref(u);

    end
    y = mean(y);

    sem = std(vertcat(data(a).sums{c}.tuningY{:}))./sqrt(size(vertcat(data(a).sums{c}.tuningY{:}),2));
    polarplot(deg2rad(x),y,condColor{c},'LineWidth',2);
    polarplot(deg2rad(repmat(x,2,1)),y+([-1;1]*sem),condColor{c},'LineWidth',2)
...
for c = conds
plot(mean(vertcat(data(a).sums{c}.tuningY{:}).*data(a).sums{c}.rPref),condColor{c})
plot(repmat(9,1,length(data(a).sums{c}.rPref))+(randn(1,length(data(a).sums{c}.rPref))*0.1),data(a).sums{c}.rPref,[condColor{c} '.'])
end
end
...
% for c = 1:length(expts)
% 
%     x = mean(vertcat(sums{c}.tuningX{:}));
%     y = mean(vertcat(sums{c}.rPref).*vertcat(sums{c}.tuningY{:}));
%     sem = std(vertcat(sums{c}.rPref).*vertcat(sums{c}.tuningY{:}))./sqrt(size(vertcat(sums{c}.tuningY{:}),2));
%     plot(x,y,condColor{c});
%     plot(repmat(x,2,1),y+([-1;1]*sem),condColor{c})
% 
% end
% xticks([-180:90:180])
...
for t = 1:3
    ...
    for c = conds
        ...
        if t == 1
            dist{t} = log2(data(a).sums{c,1}.rPref);
            yLbl = 'rPref';
            lims = [1 7 1000];
            offsetScale = 3;
            dpScale = 20;
        elseif t == 2
            dist{t} = data(a).sums{c,1}.dsi;
            yLbl = 'DSI';
            lims = [0 1 1000];
            offsetScale = 3;
            dpScale = 20;
        elseif t == 3
            dist{t} = 1-data(a).sums{c,1}.dcv;
            yLbl = '1-DCV';
            lims = [0 1 1000];
            offsetScale = 3;
            dpScale = 20;
        ...
        xBar = mean(dist{t});
        med = median(dist{t});
        sem = std(dist{t})/sqrt(length(dist{t}));
        v = violin(dist{t},0.4,lims,1,0);
        z = c*offsetScale;
        s = offsetScale/dpScale;
        plot(((exptID-(nE/2)-0.5)*s*indAnimals)+z,dist{t}','.','Color',condColor{c});
