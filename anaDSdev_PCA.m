...
for ag = 1:nAG
    ...
    %tuning metrics
    ldr{ar,ag} = D.ldr;
    ldr_mean(ar,ag) = mean(D.ldr);
    ldr_sem(ar,ag) = sem(D.ldr);
    dsi{ar,ag} = D.dsi;
    dsi_mean(ar,ag) = mean(D.dsi);
    dsi_sem(ar,ag) = sem(D.dsi);
    lor{ar,ag} = D.lor;
    lor_mean(ar,ag) = mean(D.lor);
...
%     end
%     for ag = 1:3
%         for trialNum = 1:size(rMat{ar,ag}.cTrial,1)
% 
%             t = rMat{ar,ag}.cPref(2,:);
%             c = rMat{ar,ag}.cTrial(trialNum);
%             r = rMat{ar,ag}.rTrial_norm(trialNum,:);
%             mv = meanvec(t,r);
%             % err{ar,ag}(trialNum) = rad2deg(angdiff(deg2rad(mv.angDir),deg2rad(c)));
%             err{ar,ag}(trialNum) = mod(mv.angDir - c + 180, 360) - 180;
%             % errOri{ar,ag}(trialNum) = rad2deg(angdiff(deg2rad(mv.angOri),deg2rad(mod(c,180))));
%             errOri{ar,ag}(trialNum) = mod(mv.angOri - mod(c,180) + 90, 180) - 90;
%             % figure
%             % polarplot( deg2rad(mod(t,180)) , r ,'ko','MarkerFaceColor','k');hold on
%             % polarplot(repmat(deg2rad(mv.angOri),2,1),[0;max(r)],'k','LineWidth',2)
%             % polarplot(repmat(deg2rad(mod(c,180)),2,1),[0;max(r)],'r','LineWidth',2)
%             % title(num2str(errOri{ar,ag}(trialNum)))
%         end
%         subplot(nAR,nAG,ag)
%         polarhistogram(deg2rad(err{ar,ag}),deg2rad(bins),'FaceColor',clr,'FaceAlpha',0.2);hold on
%         polarplot(deg2rad(err{ar,ag}),ones(size(err{ar,ag}))*30,[clr,'o'])
% 
%         subplot(nAR,nAG,ag+nAG)
%         polarhistogram(deg2rad(errOri{ar,ag}),deg2rad(bins),'FaceColor',clr,'FaceAlpha',0.2);hold on
%         polarplot(deg2rad(errOri{ar,ag}),ones(size(errOri{ar,ag}))*30,[clr,'o'])
%     end
% end
...
%         tDir = r.cTrial;
%         tOri = mod(tDir,180);
%         % tDir = [cos(tDir), sin(tDir)];
%         % tOri = [cos(tOri), sin(tOri)];
% 
%         miDO = mi_cont_cont(tDir,tOri,4);
% 
%         [coeff,score] = pca(r.rTrial_norm);
%         for pc = 1:size(score,2) 
%             miD = mi_cont_cont(score(:,pc),tDir,k);
%             miO = mi_cont_cont(score(:,pc),tOri,k);
%             miDir{ar,ag}(pc) = miD;
%             miOri{ar,ag}(pc) = miO;
%         end
% 
...
% Kfold = 5;
% cv = cvpartition(length(theta1),'KFold',Kfold);
% 
% maxPC = 10;
% 
% for m = 1:maxPC
...
%     for kfold = 1:Kfold
% 
%         trainIdx = training(cv,kfold);
%         testIdx  = test(cv,kfold);
% 
%         % Train linear model
%         B1 = Zm(trainIdx,:) \ S1(trainIdx,:);
%         B2 = Zm(trainIdx,:) \ S2(trainIdx,:);
% 
%         % Predict
%         S1_pred = Zm(testIdx,:) * B1;
%         S2_pred = Zm(testIdx,:) * B2;
% 
%         % Recover predicted angle
%         theta1_pred = atan2(S1_pred(:,2), S1_pred(:,1));
%         theta2_pred = atan2(S2_pred(:,2), S2_pred(:,1));
% 
%         % Circular error
%         d1 = angle(exp(1i*(theta1(testIdx) - theta1_pred)));
%         d2 = angle(exp(1i*(theta2(testIdx) - theta2_pred)));
% 
%         err1(kfold) = mean(d1.^2);
%         err2(kfold) = mean(d2.^2);
%     end
% 
%     mse1(m) = mean(err1);
%     mse2(m) = mean(err2);
...
for ag = 1:length(ageGroups)
    ...
    subplot(nAR,nAG,ag+(nAG*(ar-1)));hold on
    [coeff,score] = pca(R);
    dirClrs = hsv(length(rMat{ar,ag}.cMean));
    oriClrs = repmat(hsv(length( unique(mod(rMat{ar,ag}.cMean,180)) )),2,1);
    clrs = oriClrs;
    pc_A = 1;pc_B = 2;pc_C = 3;
    xAx = [min(score(:,pc_A)) max(score(:,pc_A))];
    yAx = [min(score(:,pc_B)) max(score(:,pc_B))]; 
    zAx = [min(score(:,pc_A)) max(score(:,pc_C))]; 
    if size(score,1)==size(rMat{ar,ag}.rTrial,1)
        plot3(score(:,pc_A),score(:,pc_B),repmat(zAx(1),size(score,1),1),'o','Color',[0.8 0.8 0.8],'MarkerFaceColor',[0.8 0.8 0.8])
        for c = 1:size(clrs,1)
        idx = rMat{ar,ag}.cTrial==rMat{ar,ag}.cMean(c);
        plot3(score(idx,pc_A),score(idx,pc_B),score(idx,pc_C),'ko','MarkerFaceColor',clrs(c,:))
        end
    else
        plot3([score(:,pc_A);score(1,pc_A)],[score(:,pc_B);score(1,pc_B)],[score(:,pc_C);score(1,pc_C)],'k','LineWidth',2)
        plot3([score(:,pc_A);score(1,pc_A)],[score(:,pc_B);score(1,pc_B)],[repmat(zAx(1),size(score,1)+1,1)],'-o','Color',[0.8 0.8 0.8],'MarkerFaceColor',[0.8 0.8 0.8])
        for c = 1:size(clrs,1)
        plot3(score(c,pc_A),score(c,pc_B),score(c,pc_C),'ko','MarkerFaceColor',clrs(c,:),'MarkerSize',10)
        end
    end
    view(3)
    if size(score,1)==size(rMat{ar,ag}.rTrial,1)
    ...
    if ar==2 && ag==1
        view(az-90,el)
    elseif size(score,1)==size(rMat{ar,ag}.rTrial,1) && ((ar==1 && ag==3 )||(ar==2 && ag==2))
        view(az+90,el)
    else
        view(az,el)
    end
    xlabel(['PC' num2str(pc_A)])
    ylabel(['PC' num2str(pc_B)])
    zlabel(['PC' num2str(pc_C)])
    xlim(xAx)
    ylim(yAx)
    zlim(zAx)
    axis square
    ...
    for b = 1:100
        uIdx = randi(height(dat{ar,ag}),1,min(nU,[],'all')); %bootstrap sample with replacement
        [~,~,~,~,explained] = pca(R(:,uIdx));
        csEx(b,:) = cumsum(explained);
    end
    mCSex = mean(csEx);
    errY = confInt(csEx);
    ys = [70 90];
    for int_y = 1:length(ys)
    int_n = 1000;
    int_x = linspace(1,length(mCSex),int_n);
    mCSex_int = interp1(mCSex,int_x);
    errY_ubInt = interp1(errY(1,:),int_x);
    errY_lbInt = interp1(errY(2,:),int_x);
    d{int_y}(ar,ag) = int_x(find(mCSex_int>ys(int_y),1));
    dUb{int_y}(ar,ag) = int_x(find(errY_ubInt>ys(int_y),1));
    dLb{int_y}(ar,ag) = int_x(find(errY_lbInt>ys(int_y),1));
    end


    subplot(1,2,1); hold on
    plot(mCSex,[arClrs{ar} agShape{ag},agLinStyl{ag}],'LineWidth',1)
    patch([1:length(mCSex) fliplr(1:length(mCSex))],[errY(1,:) fliplr(errY(2,:))],arClrs{ar},'EdgeColor','none','FaceAlpha',0.2)
    % stem(d(ar,ag),int_y,'Color',arClrs{ar},'LineStyle',agLinStyl{ag})
    % patch([dLb(ar,ag) dUb(ar,ag) dUb(ar,ag) dLb(ar,ag)],[0 0 int_y int_y],arClrs{ar},'EdgeColor','none','FaceAlpha',0.2)
    for int_y = 1:length(ys)
        yline(ys(int_y),'k--')
    end
    xlim([0,length(mCSex)])
    ...
    % plot(mCSex,['.' agLinStyl{ag}],'Color',arClrs{ar},'LineWidth',1)
    % patch([1:length(mCSex) fliplr(1:length(mCSex))],[errY(1,:) fliplr(errY(2,:))],arClrs{ar},'EdgeColor','none','FaceAlpha',0.2)
    % stem(d(ar,ag),int_y,'Color',arClrs{ar},'LineStyle',agLinStyl{ag})
    % patch([dLb(ar,ag) dUb(ar,ag) dUb(ar,ag) dLb(ar,ag)],[0 0 int_y int_y],arClrs{ar},'EdgeColor','none','FaceAlpha',0.2)
    % yline(80,'k--')
...
figure(4)
subplot(1,2,2);hold on
% plot(d(ar,:),[arClrs{ar} 'o-'])
% patch([1:nAG fliplr(1:nAG)],[dUb(ar,:) fliplr(dLb(ar,:))],arClrs{ar},'EdgeColor','none','FaceAlpha',0.2)
for int_y = 1:length(ys)
errorbar(1:nAG,d{int_y}(ar,:),d{int_y}(ar,:)-dLb{int_y}(ar,:),dUb{int_y}(ar,:)-d{int_y}(ar,:),[arClrs{ar}])
end
% ylim([4 6.5])
xlim([0 4])
box on
...
for f = 1:4
    figure(f)
    saveas(gcf,fullfile(dataFold,['anaDSdevFig' num2str(f) ttl 'Ori.fig']))
end
...
for ar = 1:length(areas)
    ...
    for ag = 1:length(ageGroups)
        subplot(2,2,1);hold on
        scatter(ldr{ar,ag},lor{ar,ag},[clr agShape{ag}])

        subplot(2,2,2);hold on
        scatter(dsi{ar,ag},osi{ar,ag},[clr agShape{ag}])
    end
    
    subplot(2,2,1);hold on
    p(ar) = errorbar(ldr_mean(ar,:),lor_mean(ar,:),lor_sem(ar,:),lor_sem(ar,:),ldr_sem(ar,:),ldr_sem(ar,:),clr);
    ...
    subplot(2,2,2);hold on
    errorbar(dsi_mean(ar,:),osi_mean(ar,:),osi_sem(ar,:),osi_sem(ar,:),dsi_sem(ar,:),dsi_sem(ar,:),clr);
