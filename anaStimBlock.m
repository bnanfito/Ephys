...
for b = blocks'
    ...
    idx = projectTbl.blockNr == b & strcmp(projectTbl.recSite,area) & strcmp(projectTbl.experimentId,anim{a});

    d{a,b} = vertcat(projectTbl(idx,:).sumStats{:});
...
expts = unique(data.pssbf.exptName);
expts = expts([1,2,3,4,5,7]);
bfDat = data.pssbf;
bfDat = bfDat(ismember(bfDat.exptName,expts),:);
durDat = vertcat(d{:,2});

bfGood = screenUnits(bfDat,'MU');
durGood = screenUnits(durDat,'MU');

disp(['median: ' num2str(median(bfDat.rPref(bfGood))) '; mad:' num2str(mad(bfDat.rPref(bfGood),1))])
disp(['median: ' num2str(median(durDat.rPref(bfGood))) '; mad:' num2str(mad(durDat.rPref(bfGood),1))])
disp(['signed rank test: p = ' num2str(signrank(durDat.rPref(bfGood)))])
disp([num2str(100*(1-(sum(durGood)/sum(bfGood)))) '% stop responding (' num2str(sum(bfGood&~durGood)) '/' num2str(sum(bfGood)) ')'])
disp(['median: ' num2str(median(durDat.rPref(durGood))) '; mad:' num2str(mad(durDat.rPref(durGood),1))])
...
unique(data.pssbf.exptName)
for i = 1:height(data.pssbf)
